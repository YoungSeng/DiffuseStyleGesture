...
if __name__ == '__main__':
    ...
    for item in os.listdir(os.path.join(root, 'Audio_normalized')):
        print(item)
        wav_path = os.path.join(root, 'Audio_normalized', item)
        wav, fs = librosa.load(wav_path, sr=16000)  # 2117849 / 16000 = 132.3655625
        mfcc = obj.sig2s2mfc_energy(wav, None)  # (13237, 15) with -1:1.275、2.275、3.375...
        print(mfcc[:, :-2].shape)       # -1 -> -2
        numpy.savez_compressed(os.path.join(root, target_path, item[:-4] + '.npz'), mfcc=mfcc[:, :-2])
